...
export async function querySalesforce(soql: string, auth: SalesforceAuth) {
  const res = await axios.get(
    `${auth.instanceUrl}/services/data/v62.0/query`,
    {
      params: { q: soql },
...
export async function createRecord(
  ...
  const res = await axios.post(
    `${auth.instanceUrl}/services/data/v62.0/sobjects/${objectType}`,
    fields,
...
async function executeTool(
  ...
  if (toolName === 'describe_object') {
    const res = await axios.get(
      `${auth.instanceUrl}/services/data/v62.0/sobjects/${toolInput.object_name}/describe`,
      { headers: { Authorization: `Bearer ${auth.accessToken}` } }
    )
